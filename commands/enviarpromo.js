...
const { CANAL_AVISOS, CARGO_LOBBY, CARGO_MODERADOR, CARGO_FS, CARGO_CRIADOR } = require('../config');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('anuncio_promo')
        .setDescription('Abre uma caixa de texto para enviar aviso ao canal de avisos.'),

    async execute(interaction) {
        // Permissão por cargo
        const allowedRoles = [CARGO_MODERADOR, CARGO_FS, CARGO_CRIADOR];
...
/*Const { SlashCommandBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, EmbedBuilder } = require('discord.js');
const { CARGO_MODERADOR, CARGO_FS, CARGO_CRIADOR, CARGO_PROMO, CANAL_PROMOCOES } = require('../config');

module.exports = (client) => {

    client.on('interactionCreate', async interaction => {
        // Comando slash
        if (interaction.isChatInputCommand() && interaction.commandName === 'enviar_promo') {
            const allowedRoles = [CARGO_MODERADOR, CARGO_FS, CARGO_CRIADOR];
            const member = await interaction.guild.members.fetch(interaction.user.id);
            const hasRole = allowedRoles.some(roleId => member.roles.cache.has(roleId));
            if (!hasRole) {
            ...
            const input = new TextInputBuilder()
                .setCustomId('promocao')
                .setLabel('Digite a promo que quer divulgar:')
                .setStyle(TextInputStyle.Paragraph)
                .setRequired(true);
        ...
        // Submissão do modal
        if (interaction.isModalSubmit() && interaction.customId === 'modal_msg') {
            ...
            const cargo = interaction.guild.roles.cache.get(CARGO_PROMO);
            const menção = cargo ? `<@&${cargo.id}>` : '';

            const canal = interaction.guild.channels.cache.get(CANAL_PROMOCOES);
    ...
    return {
        data: new SlashCommandBuilder()
            .setName('enviar_promo')
            .setDescription('Abre uma caixa de texto para enviar uma promo ao canal de promoções.')
    };
};*/
